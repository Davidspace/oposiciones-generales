...
export type LeadInput = LeadBase & {
  experiment: "administrativo-estado-c2";
  email: string;
  whatsapp: null;
  captureContract: "admin-estado-c2-email-v1";
...
export async function saveLead(input: LeadInput) {
  ...
  const contactKey = `email:${input.email.trim().toLowerCase()}`;
  const whatsappConsentAt = null;
