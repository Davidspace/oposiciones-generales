...
export const leads = sqliteTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.captureContract} = 'admin-estado-c2-email-v1'`,
    ),
    check(
      "leads_contact_contract_check",
      sql`${table.captureContract} = 'admin-estado-c2-email-v1'
        AND ${table.experiment} = 'administrativo-estado-c2'
    ...
    check(
      "leads_dates_check",
      sql`datetime(${table.consentAt}) IS NOT NULL
