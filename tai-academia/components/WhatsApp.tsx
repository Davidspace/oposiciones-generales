...
export function IconoWhatsApp({ size = 17 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="-1 -1 26 26" fill="none" stroke="currentColor" strokeWidth={1.6} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export function EnlaceWhatsApp({ href = DEFAULT_WHATSAPP_URL, size = 17 }: { href?: string; size?: number }) {
  return <a className="lm-wa" href={href} target="_blank" rel="noreferrer" aria-label="WhatsApp de Academia LORMAN"><IconoWhatsApp size={size} /><span>WhatsApp</span></a>;
}
